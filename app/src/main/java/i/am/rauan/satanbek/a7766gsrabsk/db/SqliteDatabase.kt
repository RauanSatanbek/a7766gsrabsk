...
class SqliteDatabase(context: Context) :
    ...
    override fun onCreate(db: SQLiteDatabase?) {
        ...
        var createSongsTable = "CREATE TABLE $songTable ($sId INTEGER PRIMARY KEY AUTOINCREMENT, $sKey TEXT, $sName TEXT, $sPlaylist TEXT, $sFile TEXT, $sText TEXT, $sPath TEXT, $sUrl TEXT)"

        db?.execSQL(createSongsTable)

        var createPlaylistSongsTable = "CREATE TABLE $playlistSongTable (id INTEGER PRIMARY KEY AUTOINCREMENT, $psPlaylistId INTEGER, $psSongId INTEGER," +
                "FOREIGN KEY ($psPlaylistId) REFERENCES $playlistTable ($pId), FOREIGN KEY ($psSongId) REFERENCES $songTable ($sId));"
    ...
    override fun onUpgrade(db: SQLiteDatabase?, oldVersion: Int, newVersion: Int) {
        ...
        db?.execSQL("DROP TABLE IF EXISTS $playlistSongTable")

        onCreate(db)
    }

    fun addSingerPlaylist(p: PlaylistModel): Boolean {
        Log.d("main", "GET PLAYLIST: ${getPlaylistBy(pKey, p.key.toString())}")
    ...
    fun addSong(s: SongModel): Boolean {
        ...
        Log.d("db", "addsong: ${getSongBy(sKey, s.key.toString())} ${getSongBy(sKey, s.key.toString()).key == ""}")
    ...
    fun updateSongPath(s: SongModel): Boolean {
        ...
        Log.d("main", "DB: updatePath: $success")
        return Integer.parseInt("$success") != -1
    }
        ...
        if (cursor != null) {
            if (cursor.moveToFirst()) {
                do {
                    ...
                    songs.add(SongModel("$id", key, name, playlist, file, text, path, url))

                } while (cursor.moveToNext())
            }
        }

        Log.d("main", "DB: getAllSongs: $songs")
        cursor.close()
        db.close()
    ...
    fun getLoadedSongs(): ArrayList<SongModel> {
        ...
        Log.d("main", "DB: getLoadedSongs - $songs")
    ...
    fun getSongBy(key: String, value: String): SongModel {
        ...
        if (cursor != null && cursor.moveToFirst()) {
            ...
            var path = cursor.getString(cursor.getColumnIndex(sPath))
             ...
             return SongModel("${id}", key, name, playlist, file, text, path, url)
        }

        return SongModel("",  "","", "", "", "", "", "")
    }

    fun addPlaylistAndSong(pId: Int, sId: Int): Boolean{
        var db = this.writableDatabase
        var values = ContentValues()
        values.put(psPlaylistId, pId)
        values.put(psSongId, sId)

        var success = db.insert(playlistSongTable, null, values)
    ...
    fun getSongsByPlaylistId(pId: Int): ArrayList<SongModel> {
        var songs = ArrayList<SongModel> ()

        val db = this.readableDatabase
        val query = "SELECT * FROM $playlistSongTable where $psId = ?"
        val cursor = db.rawQuery(query, arrayOf(pId.toString()))

        if (cursor != null) {
            if (cursor.moveToFirst()) {
                do {
                    var songId = cursor.getInt(cursor.getColumnIndex(psSongId))

                    var song = getSongBy(sId, songId.toString())
                    if (song.id != "") {
                        songs.add(song)
                    }
    ...
    companion object {
        private const val DB_NAME = "GaniMatebayev"
        private const val DB_VERSION = 3

        private const val SINGER_PLAYLIST = 1
        private const val OWN_PLAYLIST = 2
        private const val LOADED_PLAYLIST = 3
        ...
        private const val playlistSongTable = "playlist_song"
        private const val psId = "id"
        private const val psPlaylistId = "playlist_id"
        private const val psSongId = "song_id"
    }
}